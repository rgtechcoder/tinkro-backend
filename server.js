...
app.post("/api/chat", async (req, res) => {
  try {
    ...
    // ✅ PRICE / CHEAPEST KIT
    if (txt.includes("price") || txt.includes("cheapest") || txt.includes("sasti") || txt.includes("kit")) {
      const kits = products.filter(p => p.category?.toLowerCase() === "kit");
      if (!kits.length) return res.json({ reply: "No kits found in product list." });
    ...
    return res.json({
      reply:
        lang === "hinglish"
          ? "❓ Samajh nahi aaya. Product name ya 'sasti kit' try karo."
